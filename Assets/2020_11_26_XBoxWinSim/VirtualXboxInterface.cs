...
//Credit to : https://github.com/TybaIt/vXboxInterfaceWrap
namespace vXboxInterfaceWrap
{
    /// <summary>
    /// PInvoke for vXboxInterface.dll
    /// </summary>
    public static class VirtualXboxInterface
    {
        ...
        [DllImport("vXboxInterface.dll")]
        public static extern bool GetNumEmptyBusSlots(IntPtr nSlots);
        ...
        /// <returns><see langword="True"/> if function succeeded. Otherwise <see langword="false"/>.</returns>
        [DllImport("vXboxInterface.dll")]
        public static extern bool SetBtnGD(uint UserIndex, bool Press);
        #endregion
        ...
        /// <returns><see langword="True"/> if function succeeded. Otherwise <see langword="false"/>.</returns>
        [DllImport("vXboxInterface.dll")]
        public static extern bool GetLedNumber(uint UserIndex, uint pLed);
    ...
    public class VirtualXboxController : IDisposable
    {
...
#if DEBUG
            Debug.Log("╔══════════════════════════════════");
            Debug.Log($"║ Plugging VirtualXboxController into slot {SlotIndex} ...");
            Debug.Log($"║ Virtual Bus exists?: {VirtualXboxInterface.isVBusExists()}");
#endif

            Acquire(forceOwnership);
                
#if DEBUG
            Debug.Log($"║ Controller {SlotIndex} exists?: {VirtualXboxInterface.isControllerExists(SlotIndex)}");
            Debug.Log($"║ Controller {SlotIndex} acquired?: {Acquired}");
            Debug.Log("╚══════════════════════════════════");
#endif
            Reset();
        }
        ...
        {
            return VirtualXboxInterface.SetAxisX(SlotIndex, (short)Mathf.Clamp(value,AXIS_MIN, AXIS_MAX));
        }
        ...
        {
            return VirtualXboxInterface.SetAxisY(SlotIndex, (short)Mathf.Clamp(value, AXIS_MIN, AXIS_MAX));
        }
        ...
        {
            return VirtualXboxInterface.SetAxisRx(SlotIndex, (short)Mathf.Clamp(value, AXIS_MIN, AXIS_MAX));
        }
        ...
        {
            return VirtualXboxInterface.SetAxisRy(SlotIndex, (short)Mathf.Clamp(value, AXIS_MIN, AXIS_MAX));
        }
